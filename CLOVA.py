...
df = pd.read_excel("partner_data.xlsx", skiprows=2)
rubric_df = pd.read_excel("Criteria.xlsx")

rubric_lookup = {}
for idx, row in rubric_df.iterrows():
    key_q = row["Key Questions"]
    rubric_text = row["진단 기준 정의"]
    rubric_lookup[key_q.strip()] = rubric_text.strip()
...
def evaluate_answer(question, answer):
    ...
    rubric = rubric_lookup.get(question.strip(), "No rubric found for this question.")
    ...
    system_prompt = (
        ...
        "예를 들어, 일반적으로는 현장 인력이 필요한 활동을 즉각 대처 가능한 24시간 유선 운영 등으로 효과적으로 대체하고 있다면, 이는 창의적인 방식으로 높은 성과를 낸 것으로 판단하여 가능한 높은 점수를 부여해 주세요."
